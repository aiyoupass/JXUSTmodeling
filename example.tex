...
\keyword{手册；数模；全国大学生数学建模竞赛；\TeX{}；\LaTeX{}；\XeLaTeX{}；}
\begin{document}
...
\begin{tabularx}{\textwidth}{  c   @{\hspace{1pc}}  |  @{\hspace{1pc}}   Y   }
  % @{\hspace{1pc}}表示一个长度为1pc的空白
  % c和 Y 是两种不同的居中对齐方式,可自行更改尝试
  % | 表示在两列中间画一条竖线
  \Xhline{0.08em}
  符号 & 符号说明\\
  \Xhline{0.05em}
...
\begin{figure}[htbp]
  ...
  %width=0.4\textwidth是设置图片宽度为文本宽度的0.4
  %若图片再figures文件夹中，路径应写 figures/tupian.png
...
\begin{figure}[htbp]
  \centering
  \subfloat[子图注1]{\label{sub-fig-1}% 为子图加交叉引用
  \begin{minipage}{0.4\textwidth}
    \centering
    \includegraphics[width=\textwidth]{example-image-a}
  \end{minipage}
  }
  \qquad %这个是两幅图片中间那个距离（大概是四个空格）
  \subfloat[子图注2]{%
  \begin{minipage}{0.4\textwidth}
    \centering
